...
<body>
	...
	<div class="container container-board">
		...
		<div class="row">
			...
			<div class="col-3 header-board">
				<p>
					<a href="/download.fi?ori_name=${file.ori_name}&sys_name=${file.sys_name}">${file.ori_name}</a>
				</p>
			</div>
		</div>
		<div class="row">
			...
			<div class="col-9 text-center">
				<img class="" style="width: 200px" src="/files/${file.sys_name}">
			</div>
		</div>
		<div class="row">
			...
			<c:if test="${not empty replyList}">
				...
				<c:forEach items="${replyList}" var="reply">
					<div class="co1-12 body-header-reply">
						<span class="writer-reply">${reply.writer_nickname}</span> <span class="date-reply">${reply.written_date}</span>
					</div>
					...
					<c:if test="${reply.writer_id eq loginSession.id}">
						<%--댓글작성자 id와 로그인한 사람의 id가 같다면 수정삭제버튼 보이기 --%>
